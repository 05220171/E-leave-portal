...
<div class="container mt-4">
    ...
    {{-- Using a structure similar to your HOD dashboard's card, but for a form --}}
    <div class="card shadow-sm mb-4"> {{-- Standard Bootstrap card can be styled by your general .card rules in student.css --}}
        ...
        <form method="POST" action="{{ route('superadmin.leave-types.store') }}" class="needs-validation" novalidate>
            @csrf
            <div class="card-body" style="padding: 1.5rem;"> {{-- Added some padding to card-body --}}
                @if ($errors->any())
                    <div class="custom-alert custom-alert-danger">
                        <ul class="mb-0 ps-3"> {{-- Added padding-start for list --}}
                ...
                <div class="mb-4 form-check"> {{-- Increased bottom margin --}}
            ...
            <div class="card-footer text-end bg-light"> {{-- Light background for footer --}}
                <a href="{{ route('superadmin.leave-types.index') }}" class="custom-btn custom-btn-secondary me-2">Cancel</a>
                <button type="submit" class="custom-btn custom-btn-primary"><i class="fas fa-save me-1"></i> Create Leave Type</button>
            </div>
        </form>
    </div>
</div>
